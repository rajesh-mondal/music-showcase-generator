...
class Generator {
    ...
    public function __construct( string $locale ) {
        $targetLocale = $locale;
        $data = $this->loadDataFromFile( $targetLocale );

        // If the requested locale failed to load, try falling back to en_US.
        if ( !$data ) {
            $targetLocale = 'en_US';
            $data = $this->loadDataFromFile( $targetLocale );
        }

        // If the default (en_US) also failed
        if ( !$data ) {
            throw new \RuntimeException(
                "Critical Error: Could not load data for locale '{$locale}' or the fallback 'en_US'. Check your 'src/Locales' directory."
            );
        }
